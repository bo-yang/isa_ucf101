function [] = extract_unsupervised_training_data_hw2(train_video_dir, train_list, result_filename, params)

spatial_size = params.spatial_size;
temporal_size = params.temporal_size;
num_patches = params.num_patches;

%dirlist = dir([train_video_dir,'*.avi']); %%% TEST ONLY %%%
dirlist = train_list;
%num_clips = length(dirlist);
%train_filenames = cell(num_clips, 1); 
%for i = 1 : num_clips
%    train_filenames{i} = dirlist(i);
%end
train_filenames = dirlist;
...
Mtmp={};
parfor i=1:num_clips
    filename = [path, filenames{i}];
    fprintf('loading clip: %s\n', filename);
    Mtmp{i} = loadclip_3dm(filename, sp_size, 0, 0);
end
