%% set paths
vid_file_string='/home/boyang/UCF101/UCF-101/';
...
%% extract training list
train_list=[];
train_list_dir=[path, '/ucfTrainTestlist/']; 
tlists=dir([train_list_dir,'train*']);
for i=1:length(tlists)
	fid=fopen([train_list_dir,tlists(i).name]);
	tmp=textscan(fid,'%s%d');
	train_list=[train_list;tmp{1}];
end
...
for i = 1:network.num_isa_layers %1 and 2 layers (since 3rd layer is the same size)       
    ...
    extract_unsupervised_training_data_hw2(vid_file_string, train_list, data_path_file_name{i}, params_ex); 
end
