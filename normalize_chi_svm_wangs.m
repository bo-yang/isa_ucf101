function [predict, mean_ap, mean_acc] = normalize_chi_svm_wangs(Xtrain, Xtest, norm_type,  ...
...
if(unscramble)
    ...
    for i = 2:length(sorted_test_files)
        if strcmp(sorted_test_files(i), sorted_test_pred_labelsfiles(i-1)) == 0
            counter= counter +1;
...
%actions = t[labels,actions]=textread([info_path,'classInd.txt'],'%d %s');
[~,actions]=textread([infopath,'classInd.txt'],'%d %s');
l = length(actions);

%%------------------ subsample training data ---------------------------accuracy=calc_recg_accuracy(all_test_files,all_svm_pred(ini_idx).labels, params.infopath);
...
average_ap = 0;
average_acc = 0;
predict=struct('labels',[],'accuracy',0);
...
for label=1:l    
    ...
    if sum(Ytrain_new)==0 
        continue % No training case of this action
    end
    ...
    [pred_labels,accuracy,prob_estimates] = svmpredict(Ytest_new, Ktest, model, '-b 1');
    idx=find(pred_labels>0);
    predict.labels=[predict.labels pred_labels*label];
...
predict.accuracy=mean_acc; %calc_recg_accuracy(all_test_files,predict.labels,infopath);
